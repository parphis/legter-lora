...
void parseGGAString(char *str, gps_type *ptr_coords)
{
    const char s[4] = ",";
    char *tok = 0;
    char *latitude = (char*)calloc(1024, sizeof(char));
    char *longitude = (char*)calloc(1024, sizeof(char));
    ...
    while (tok != 0) {
        ...
        if ( (gpgga==1) && (i>=2 && i<=5) ) {
            switch (i)
            {
                case 2: strcpy(latitude,tok); break;
                case 3: strcpy(ptr_coords->dir_lat,tok); break;
                case 4: strcpy(longitude,tok); break;
                case 5: strcpy(ptr_coords->dir_lon,tok); break;
                default: ;
            }
        }
    ...
    if (gpgga==1) {
        ...
        printf("%f %s, %f %s\n", ptr_coords->latitude, ptr_coords->dir_lat, ptr_coords->longitude, ptr_coords->dir_lon);
        fflush(stdout);
#endif
    }
...
/**
 * Open the ttyS0 port and return the descriptior.
 * TODO port parameters to config file.
 *
 * @returns int
 */
int openPort()
...
/**
 * Closes the port give by a descriptor.
 *
 * @param file descriptio int
 */
